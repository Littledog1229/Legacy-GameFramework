...
public sealed class UIBatch : RenderBatch<UIBatch.UIVertex> {
    ...
    public void drawRect(Rect rect, Color4 tint) {
        verifySpace(4, 6);
        
        var min = new Vector2(rect.X, rect.Y);
        var max = min + new Vector2(rect.Width, rect.Height);
    ...
    protected override Shader getShader() => RenderManager.getOrCreateShader("UIBatch", "Resource.Shader.UIShader.glsl", Engine.EngineAssembly);
}